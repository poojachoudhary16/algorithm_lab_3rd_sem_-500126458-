...
struct Node* iterativeInsert(struct Node* root, int data) {struct Node* newNode =
createNode(data);
if (root == NULL) return newNode; //if tree is empty
...
struct Node* recursiveInsert(struct Node* root, int data) {if (root == NULL) return
createNode(data); //tree is empty
...
struct Node* root2 = NULL; // For recursive insertions// Measure time for iterative insertion
